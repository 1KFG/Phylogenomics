...
if [ $ctCDS -gt 0 ]; then
perl scripts/combine_fasaln.pl -o all_${count}.${MARKERS}.denovocds.fasaln -ext denovo_cdsaln.trim -if fasta -of fasta -d aln/$MARKERS -expected $EXPECTEDNAMES > all_${count}.${MARKERS}.denovocds.partitions.txt
